...
exports.assertInnerError = assertInnerError;
...
function assertInnerError(error) {
    ...
    return new Error(`Inner error is not an instance of Error. Object:\n\t${stringifyData(error)}`);
}
/**
...
function createErrorMessage(methodName, errorMessage, inputData, innerError) {
    const internalErrorMessage = innerError?.message !== undefined
        ? `\n-Internal error: \n\t${innerError.message}`
        : '';
