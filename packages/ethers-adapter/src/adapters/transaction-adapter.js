...
exports.adaptTransaction = adaptTransaction;
/**
 ...
 */
async function calculateDefaultMaxPriorityFeePerGas(provider, baseFee) {
    ...
    if (feeHistory.reward !== null &&
        ...
        percentile75 = BigInt(await provider.thorClient.gas.getMaxPriorityFeePerGas());
    }
 ...
 */
async function adaptTransaction(tx, hardhatVeChainProvider) {
    // Check if Galactica fork has happened
    const galacticaHappened = await hardhatVeChainProvider.thorClient.forkDetector.isGalacticaForked('best');
    ...
    if (!galacticaHappened &&
        ...
        throw new Error('Dynamic fee transaction (maxFeePerGas/maxPriorityFeePerGas) is not allowed before Galactica fork');
    }
    ...
    if (hasMaxPriorityFeePerGas && hasGasPrice && !hasMaxFeePerGas) {
        throw new Error('Invalid parameter combination: maxPriorityFeePerGas and gasPrice cannot be used together without maxFeePerGas');
    }
    // Case 4: maxFeePerGas + gasPriceCoef (error)
    if (hasMaxFeePerGas && hasGasPrice && !hasMaxPriorityFeePerGas) {
        throw new Error('Invalid parameter combination: maxFeePerGas and gasPrice cannot be used together without maxPriorityFeePerGas');
    }
    ...
    if (galacticaHappened && !hasMaxFeePerGas && !hasMaxPriorityFeePerGas) {
        const bestBlockBaseFeePerGas = await hardhatVeChainProvider.thorClient.blocks.getBestBlockBaseFeePerGas();
        ...
        const maxPriorityFeePerGas = await calculateDefaultMaxPriorityFeePerGas(hardhatVeChainProvider, baseFee);
    ...
    if (galacticaHappened && (hasMaxFeePerGas || hasMaxPriorityFeePerGas)) {
        if (!hasMaxFeePerGas || !hasMaxPriorityFeePerGas) {
            throw new Error('Both maxFeePerGas and maxPriorityFeePerGas must be set for dynamic fee transactions');
        }
        return {
            ...tx,
