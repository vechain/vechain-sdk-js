"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.ethGetBlockByHash = void 0;
const sdk_core_1 = require("@vechain/sdk-core");
const utils_1 = require("../../../../../utils");
const sdk_errors_1 = require("@vechain/sdk-errors");
const formatter_1 = require("../../../formatter");
const eth_chainId_1 = require("../eth_chainId");
/**
 * RPC Method eth_getBlockByHash implementation
 *
 * @link [eth_getBlockByHash](https://ethereum.github.io/execution-apis/api-documentation/)
 *
 * @param thorClient - The thor client instance to use.
 * @param params - The standard array of rpc call parameters.
 *                 * params[0]: The block hash of block to get.
 *                 * params[1]: The transaction hydrated detail flag. If true, the block will contain the transaction details, otherwise it will only contain the transaction hashes.
 * @returns the block at the given block hash formatted to the RPC standard or null if the block does not exist.
 * @note Ethereum block hash is passed to Thor as the block id.
 * @throws {JSONRPCInvalidParams, JSONRPCInternalError}
 */
const ethGetBlockByHash = async (thorClient, params) => {
    // Input validation
    if (params.length !== 2 ||
        typeof params[0] !== 'string' ||
        !sdk_core_1.ThorId.isValid(params[0]) ||
        typeof params[1] !== 'boolean')
        throw new sdk_errors_1.JSONRPCInvalidParams('eth_getBlockByHash', `Invalid input params for "eth_getBlockByHash" method. See ${utils_1.RPC_DOCUMENTATION_URL} for details.`, { params });
    try {
        const [blockHash, isTxDetail] = params;
        let chainId = '0x0';
        // If the transaction detail flag is set, we need to get the chain id
        if (isTxDetail) {
            chainId = await (0, eth_chainId_1.ethChainId)(thorClient);
        }
        const block = isTxDetail
            ? await thorClient.blocks.getBlockExpanded(blockHash)
            : await thorClient.blocks.getBlockCompressed(blockHash);
        return block !== null
            ? formatter_1.blocksFormatter.formatToRPCStandard(block, chainId)
            : null;
    }
    catch (e) {
        throw new sdk_errors_1.JSONRPCInternalError('eth_getBlockByHash()', 'Method "eth_getBlockByHash" failed.', {
            params: (0, sdk_errors_1.stringifyData)(params),
            url: thorClient.httpClient.baseURL,
            innerError: (0, sdk_errors_1.stringifyData)(e)
        });
    }
};
exports.ethGetBlockByHash = ethGetBlockByHash;
