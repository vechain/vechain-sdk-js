...
exports.ethSignTransaction = void 0;
 ...
 */
const ethSignTransaction = async (thorClient, params, provider) => {
    // Input validation
    if (params.length !== 1 || typeof params[0] !== 'object')
        throw new sdk_errors_1.JSONRPCInvalidParams('eth_signTransaction', `Invalid input params for "eth_signTransaction" method. See ${utils_1.RPC_DOCUMENTATION_URL} for details.`, { params });
    // Provider must be defined
    if (provider?.wallet === undefined) {
        throw new sdk_errors_1.JSONRPCInvalidParams('eth_signTransaction', 'Provider must be defined with a wallet. Ensure that the provider is defined and connected to the network.', { provider });
    }
    // From field is required
    if (params[0].from === undefined) {
        throw new sdk_errors_1.JSONRPCInvalidParams('eth_signTransaction', 'From field is required in the transaction object.', { provider });
    }
    // Input params
    ...
    try {
        ...
        throw new sdk_errors_1.JSONRPCInternalError('eth_signTransaction()', 'Method "eth_signTransaction" failed.', {
            params: (0, sdk_errors_1.stringifyData)(params),
            url: thorClient.httpClient.baseURL
        }, error);
    }
};
exports.ethSignTransaction = ethSignTransaction;
