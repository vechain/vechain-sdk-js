...
exports.getCriteriaSetForInput = exports.formatToLogsRPC = void 0;
 ...
 */
const formatToLogsRPC = (eventLogs) => {
    // Final RPC event logs formatted
    return eventLogs.map((eventLog) => {
        return {
            address: eventLog.address,
            blockHash: eventLog.meta.blockID,
            blockNumber: sdk_core_1.HexInt.of(eventLog.meta.blockNumber).toString(),
            data: eventLog.data,
            logIndex: '0x0',
...
exports.formatToLogsRPC = formatToLogsRPC;
/**
 ...
 */
const _scatterArrayTopic = (criteriaTopicsArray, address) => {
    return {
        address,
...
const getCriteriaSetForInput = (criteria) => {
    ...
    const eventsCriteriaToFlat = criteriaAddress.map((addr) => {
        return getTopicsPerAddress(addr, criteria.topics ?? []);
    });
    // Flat the array
    return eventsCriteriaToFlat.flat();
};
exports.getCriteriaSetForInput = getCriteriaSetForInput;
/**
 ...
 * @returns {EventCriteria[]} An array of EventCriteria objects.
 */
const getTopicsPerAddress = (address, topics) => {
