...
import { addressUtils, _Hex0x } from '@vechain/sdk-core';
...
class DebugModule {
    ...
    public async traceContractCall(
        ...
        if (
            ...
            !_Hex0x.isValid(input.contractInput.data, true)
        )
            throw new InvalidDataType(
                'DebugModule.traceContractCall()',
        ...
        if (
            ...
            !_Hex0x.isValid(input.contractInput.value)
        ) {
            throw new InvalidDataType(
                'DebugModule.traceContractCall()',
    ...
    private validateTarget(
        ...
        if (!_Hex0x.isThorId(target.blockID)) {
            throw new InvalidDataType(
        ...
        if (typeof target.transaction === 'string') {
            if (!_Hex0x.isThorId(target.transaction))
                throw new InvalidDataType(
                    'DebugModule.validateTarget()',
