...
class ContractFactory {
     ...
     */
    constructor(abi, bytecode, signer, contractsModule) {
        this.abi = abi;
    ...
    async startDeployment(deployParams, options) {
        ...
        const deployContractClause = sdk_core_1.Clause.deployContract(sdk_core_1.HexUInt.of(this.bytecode), deployParams);
        // Estimate the gas cost of the transaction
        const gasResult = await this.contractsModule.transactionsModule.estimateGas([deployContractClause], await this.signer.getAddress(), options // Pass through EstimateGasOptions including gasPadding
        );
        const txBody = await this.contractsModule.transactionsModule.buildTransactionBody([deployContractClause], gasResult.totalGas, options);
        // Sign the transaction
        const signedTx = await this.signer.signTransaction(signer_1.signerUtils.transactionBodyToTransactionRequestInput(txBody, await this.signer.getAddress()));
        // Send the signed transaction to the blockchain
        this.deployTransaction =
            await this.contractsModule.transactionsModule.sendTransaction(sdk_core_1.Transaction.decode(sdk_core_1.HexUInt.of(signedTx.slice(2)).bytes, true));
        return this;
    }
    /**
    ...
    async waitForDeployment() {
        ...
        if (this.deployTransaction?.id === undefined) {
            throw new sdk_errors_1.CannotFindTransaction('ContractFactory.waitForDeployment()', 'Cannot find a contract deployment transaction', {
                networkUrl: this.contractsModule.transactionsModule.blocksModule
                    .httpClient.baseURL
            });
        }
        ...
        if (transactionReceipt?.outputs[0]?.contractAddress === null ||
            transactionReceipt?.outputs[0]?.contractAddress === undefined) {
            throw new sdk_errors_1.ContractDeploymentFailed('ContractFactory.waitForDeployment()', 'Contract deployment failed.', {
                deployTransaction: this.deployTransaction
            });
        }
        // Construct and return a new Contract instance
        return new contract_1.Contract(transactionReceipt?.outputs[0].contractAddress, this.abi, this.contractsModule, this.signer, transactionReceipt);
    }
    /**
