...
const chainTagCache = new WeakMap<ThorClient, number>();
...
class NodesModule {
     ...
     */
    public async getChaintag(thorClient: ThorClient): Promise<number> {
        const cached = chainTagCache.get(thorClient);
        ...
        chainTagCache.set(thorClient, chainTag);
        return chainTag;
    }
}
