...
type ExecuteCallMock = jest.MockedFunction<ContractsModule['executeCall']>;

interface MockedContract {
    contract: Contract<SimpleAbi>;
    executeCall: ExecuteCallMock;
}

function createMockedContract(): MockedContract {
    const executeCall: ExecuteCallMock = jest.fn();
...
describe('Contract read options', () => {
    test('uses revision configured via setContractReadOptions when no clause override is provided', async () => {
        ...
        expect(executeCall.mock.calls[0][3]?.revision?.toString()).toBe(
            '0x1234'
        );
    });
    ...
    test('clause-level revision overrides contract-level revision without clearing it', async () => {
        ...
        expect(executeCall.mock.calls[1][3]?.revision?.toString()).toBe(
            '0xaaaa'
        );
    });
});
