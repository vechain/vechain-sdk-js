...
describe('Block', () => {
    ...
    test('fromResponse mirrors regular block response', () => {
        ...
        expect(block?.id.toString()).toBe(BASIC_BLOCK_JSON.id);
        expect(block?.size).toBe(BASIC_BLOCK_JSON.size);
        expect(block?.parentID.toString()).toBe(BASIC_BLOCK_JSON.parentID);
        expect(block?.timestamp).toBe(BASIC_BLOCK_JSON.timestamp);
        expect(block?.gasLimit).toBe(BigInt(BASIC_BLOCK_JSON.gasLimit));
        expect(block?.beneficiary.toString()).toBe(BASIC_BLOCK_JSON.beneficiary);
        ...
        expect(block?.txsRoot.toString()).toBe(BASIC_BLOCK_JSON.txsRoot);
        expect(block?.txsFeatures).toBe(BASIC_BLOCK_JSON.txsFeatures);
        expect(block?.stateRoot.toString()).toBe(BASIC_BLOCK_JSON.stateRoot);
        expect(block?.receiptsRoot.toString()).toBe(BASIC_BLOCK_JSON.receiptsRoot);
        expect(block?.com).toBe(BASIC_BLOCK_JSON.com);
        expect(block?.signer.toString()).toBe(BASIC_BLOCK_JSON.signer);
