...
const createMockPublicClient = (): jest.Mocked<PublicClient> => ({
    ...
    simulateCalls: jest.fn().mockResolvedValue({ 
        results: [{ 
    ...
    watchEvent: jest.fn().mockReturnValue(() => {}),
    getLogs: jest.fn().mockResolvedValue([]),
    createEventFilter: jest.fn().mockReturnValue({ id: 'filter-1' })
} as any);
...
describe('getContract function', () => {
    const contractAddress = '0x0000000000000000000000000000456E65726779' as const;
    ...
    test('should create proper event interfaces', () => {
        ...
        expect(typeof contract.events.Approval.createEventFilter).toBe('function');
    });
