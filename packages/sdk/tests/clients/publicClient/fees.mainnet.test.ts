...
describe('PublicClient - Fee Estimation Methods', () => {
    const publicClient = new PublicClient(ThorNetworks.MAINNET);
    ...
    describe('getFeeHistory', () => {
        test('should retrieve fee history for 1 block', async () => {
            ...
            console.log('Base Fee Per Gas Array Length: ' + feeHistory.baseFeePerGas.length);
            console.log('Gas Used Ratio Array Length: ' + feeHistory.gasUsedRatio.length);
        ...
        test('should retrieve fee history for multiple blocks', async () => {
            ...
            console.log('Data retrieved successfully for ' + blockCount + ' blocks');
        }, 10000);
    });
    ...
    describe('estimateFeePerGas', () => {
        test('should estimate fee per gas', async () => {
            ...
            if (feePerGas !== undefined) {
                ...
                console.log('Estimated Fee Per Gas: ' + feePerGas.toString() + ' wei');
                console.log('Fee in VTHO: ' + (Number(feePerGas) / 1e18).toFixed(8) + ' VTHO');
            } else {
    ...
    describe('estimateGas', () => {
        test('should estimate gas for contract call', async () => {
            const gasEstimate = await publicClient.estimateGas(sampleContractCall);
            ...
            console.log('Gas estimate results: ' + gasEstimate.length + ' clauses');
            console.log('First clause gas used: ' + firstResult.gasUsed.toString());
        ...
        test('should estimate gas for transfer transaction', async () => {
            ...
            const estimate = await publicClient.estimateGas(transferTransaction);
            ...
            if (result.reverted) {
                console.log('Transfer reverted (likely due to insufficient balance in test environment)');
            }
        }, 10000);
    });
    ...
    describe('estimateMaxPriorityFeePerGas', () => {
        test('should estimate max priority fee per gas', async () => {
            const priorityFeeResponse = await publicClient.estimateMaxPriorityFeePerGas();
            ...
            expect(typeof priorityFeeResponse.maxPriorityFeePerGas).toBe('bigint');
            expect(priorityFeeResponse.maxPriorityFeePerGas).toBeGreaterThanOrEqual(0n);
            
            console.log('Priority Fee: ' + priorityFeeResponse.maxPriorityFeePerGas.toString());
        }, 10000);
    });
    ...
    describe('fee calculation helpers', () => {
        test('should calculate transaction costs', async () => {
            ...
            if (feePerGas !== undefined) {
                ...
                console.log('Total Cost in VET: ' + (Number(totalCost) / 1e18).toFixed(8) + ' VET');
            } else {
                console.log('Fee per gas estimation returned undefined, cannot calculate total cost');
            }
        }, 15000);
    });
