...
class RLPCodecTransactionRequest {
    ...
    public static encodeSignedTransactionRequest(
        ...
        if (isDynamicFee) {
            const encodedTx = RLPCodecTransactionRequest.encodeSignedDynamicFeeBodyField(
                body,
    ...
    public static encodeTransactionRequest(
        ...
        if (isDynamicFee) {
            const encodedTx = RLPCodecTransactionRequest.encodeUnsignedDynamicFeeBodyField(body);
            // For EIP-1559 transactions, prepend the transaction type (0x02)
            return new Uint8Array([0x02, ...encodedTx]);
        }
    ...
    private static encodeUnsignedDynamicFeeBodyField(body: RLPValidObject): Uint8Array {
        return RLPProfiler.ofObject(
            body,
    ...
    private static mapBody(
        ...
        if (transactionRequest.isDynamicFee()) {
            return {
                ...
                maxPriorityFeePerGas: transactionRequest.maxPriorityFeePerGas ?? 0n
