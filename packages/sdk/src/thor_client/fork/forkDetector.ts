...
const FQP = 'packages/sdk/src/thor_client/fork/forkDetector';
    ...
    public async isGalacticaForked(
        ...
        if (!Revision.isValid(revision)) {
            throw new IllegalArgumentError(
                `${FQP}.GalacticaForkDetector.isGalacticaForked()`,
        ...
        const response = await this.httpClient.get(
            { path: thorest.blocks.get.BLOCK_DETAIL(String(revision)) },
            { query: buildQuery({ expanded: true }).query }
        );
        const block = (await response.json()) as BlockDetail | null;
