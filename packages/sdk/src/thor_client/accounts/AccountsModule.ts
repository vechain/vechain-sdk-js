...
import { type Address, Hex, HexUInt } from '@vcdm';
...
class AccountsModule {
    ...
    public async getAccount(
        ...
        return new AccountDetail(
            (await this.httpClient.get(
                { path: thorest.accounts.get.ACCOUNT_DETAIL(address.toString()) },
                buildQuery({ revision })
    ...
    public async getBytecode(
        ...
        const result = (await this.httpClient.get(
            { path: thorest.accounts.get.ACCOUNT_BYTECODE(address.toString()) },
            buildQuery({ revision })
        )) as unknown as ResponseBytecode;
    ...
    public async getStorageAt(
        ...
        const revision = options?.revision?.toString();
        const result = (await this.httpClient.get(
            { path: thorest.accounts.get.STORAGE_AT(address.toString(), pos) },
            buildQuery({ pos, revision })
        )) as unknown as ResponseStorage;
