...
const FQP = 'packages/sdk/src/thor_client/nodes/nodes-module';
...
class NodesModule {
    ...
    public async getNodes(): Promise<ConnectedPeer[]> {
        const response = await this.blocksModule.httpClient.get(
            { path: thorest.nodes.get.NODES() },
            { query: '' }
        );
        const nodes = (await response.json()) as ConnectedPeer[] | null;
        return nodes ?? [];
    }
    ...
    private readonly getTimestampFromBlock = (
        ...
        if (
            ...
            throw new IllegalArgumentError(
                `${FQP}.NodesModule.getTimestampFromBlock()`,
