...
const FQP = 'packages/sdk/src/thor_client/debug/DebugModule';
...
class DebugModule {
     ...
     *
     * @throws IllegalArgumentError If {@link TransactionTraceTarget} `input.target` has a negative `clauseIndex` or `transaction` property.
     */
    public async retrieveStorageRange(input: {
        ...
        return (await this.httpClient.post(
            { path: thorest.debug.post.RETRIEVE_STORAGE_RANGE() },
            { query: '' },
            {
                target: parsedTarget,
        ...
        )) as unknown as RetrieveStorageRange;
    }
    ...
    public async traceContractCall(
        ...
        // Send request
        return (await this.httpClient.post(
            { path: thorest.debug.post.TRACE_CONTRACT_CALL() },
            { query: '' },
            {
        ...
        )) as unknown as TraceReturnType<typeof name>;
    }
    ...
    public async traceTransactionClause(
        ...
        // Send request
        return (await this.httpClient.post(
            { path: thorest.debug.post.TRACE_TRANSACTION_CLAUSE() },
            { query: '' },
            {
                target: parsedTarget,
                name,
