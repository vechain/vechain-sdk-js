...
class Secp256k1 {
    ...
    static compressPublicKey(publicKey) {
        ...
        if (prefix === Secp256k1.UNCOMPRESS_PREFIX) {
            ...
            return nc_utils.concatBytes(Uint8Array.of(Secp256k1.COMPRESSED_PREFIX + isYOdd), x);
        }
        else {
            // Compressed.
            return publicKey;
    ...
    static derivePublicKey(privateKey, isCompressed = true) {
        ...
        if (Secp256k1.isValidPrivateKey(privateKey)) {
            return secp256k1_1.secp256k1.getPublicKey(privateKey, isCompressed);
        }
        throw new sdk_errors_1.InvalidSecp256k1PrivateKey('Secp256k1.derivePublicKey', 'Invalid private key given as input. Ensure it is a valid 32-byte secp256k1 private key.', undefined);
    }
    /**
    ...
    static async generatePrivateKey() {
        return await new Promise((resolve, reject) => {
            try {
                ...
                reject(new sdk_errors_1.InvalidSecp256k1PrivateKey('Secp256k1.generatePrivateKey', 'Private key generation failed: ensure you have a secure random number generator available at runtime.', undefined, e));
            }
        });
    }
    /**
    ...
    static inflatePublicKey(publicKey) {
        ...
        if (prefix !== Secp256k1.UNCOMPRESS_PREFIX) {
            ...
            const p = secp256k1_1.secp256k1.ProjectivePoint.fromAffine(secp256k1_1.secp256k1.ProjectivePoint.fromHex(HexUInt_1.HexUInt.of(x).digits).toAffine());
     ...
     */
    static isValidMessageHash(hash) {
     ...
     */
    static randomBytes(bytesLength = 32) {
        try {
    ...
    static recover(messageHash, sig) {
        ...
        if (!Secp256k1.isValidMessageHash(messageHash)) {
            throw new sdk_errors_1.InvalidSecp256k1MessageHash('Secp256k1.recover', 'Invalid message hash given as input. Ensure it is a valid 32-byte message hash.', { messageHash });
        }
        if (sig.length !== Secp256k1.SIGNATURE_LENGTH)
            throw new sdk_errors_1.InvalidSecp256k1Signature('Secp256k1.recover', 'Invalid signature given as input. Length must be exactly 65 bytes.', { signature: sig });
        const recovery = sig[64];
        if (recovery !== 0 && recovery !== 1)
            throw new sdk_errors_1.InvalidSecp256k1Signature('Secp256k1.recover', 'Invalid signature recovery value. Signature bytes at position 64 must be 0 or 1.', { signature: sig, recovery });
        return secp256k1_1.secp256k1.Signature.fromCompact(sig.slice(0, 64))
    ...
    static sign(messageHash, privateKey) {
        // Check if the message hash is valid.
        if (!Secp256k1.isValidMessageHash(messageHash)) {
            throw new sdk_errors_1.InvalidSecp256k1MessageHash('Secp256k1.sign', 'Invalid message hash given as input. Ensure it is a valid 32-byte message hash.', { messageHash });
        }
        // Check if the private key is valid.
        if (!Secp256k1.isValidPrivateKey(privateKey)) {
            throw new sdk_errors_1.InvalidSecp256k1PrivateKey('Secp256k1.sign', 'Invalid private key given as input. Ensure it is a valid 32-byte secp256k1 private key.', undefined);
        }
        const sig = secp256k1_1.secp256k1.sign(messageHash, privateKey);
        return nc_utils.concatBytes(nc_utils.numberToBytesBE(sig.r, 32), nc_utils.numberToBytesBE(sig.s, 32), nc_utils.numberToVarBytesBE(sig.recovery));
    }
}
