...
class ABI implements VeChainDataModel<ABI> {
     ...
     * @param abi ABI to get the first decoded value from.
...
const abi2 = {
    ...
    decode: <ReturnType>(
        ...
        ABI.getFirstDecodedValue<ReturnType>(
            ABI.ofEncoded(types as string, data)
        )
};
