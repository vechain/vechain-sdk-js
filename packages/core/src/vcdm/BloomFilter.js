...
const Blake2b256_1 = require("./hash/Blake2b256");
...
class BloomFilter {
    ...
    get n() {
        ...
        throw new sdk_errors_1.InvalidDataType('BloomFilter.n', 'not in the safe number range', {
            bytes: this.bytes,
            k: this.k
        });
    }
     ...
     */
    contains(key) {
        return distribute(hash(key instanceof Hex_1.Hex ? key.bytes : key), this.k, this.bytes.byteLength * 8, (index, bit) => {
    ...
    join(other) {
        if (this.k !== other.k) {
            throw new sdk_errors_1.InvalidOperation('BloomFilter.join', 'different k values', {
                this: this,
                other
            });
        }
        if (this.bytes.length !== other.bytes.length) {
            throw new sdk_errors_1.InvalidOperation('BloomFilter.join', 'different length values', {
                this: this,
                other
            });
        }
...
class BloomFilterBuilder {
    ...
    add(...keys) {
        for (const key of keys) {
            this.hashMap.set(hash(key instanceof Hex_1.Hex ? key.bytes : key), true);
        }
        return this;
    }
    /**
     ...
     */
    build(k = BloomFilterBuilder.DEFAULT_K, m = BloomFilter.computeBestBitsPerKey(k)) {
 ...
 */
function distribute(hash, k, m, collision) {
 ...
 */
function hash(key) {
    return Number(nc_utils.bytesToNumberBE(Blake2b256_1.Blake2b256.of(key).bytes.slice(0, 4)));
}
