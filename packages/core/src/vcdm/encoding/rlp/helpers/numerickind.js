...
exports.validateNumericKindData = exports.encodeBigIntToBuffer = exports.decodeBufferToNumberOrHex = exports.assertValidNumericKindBuffer = void 0;
 ...
 * @throws {InvalidRLP}
 */
const validateNumericKindData = (data, context) => {
    // Input data must be either a number or a string.
    if (typeof data !== 'number' && typeof data !== 'string') {
        throw new sdk_errors_1.InvalidRLP('validateNumericKindData()', `Validation error: Input in ${context} must be a string or number.`, {
            context,
            data: {
                data
            }
...
exports.validateNumericKindData = validateNumericKindData;
/**
 ...
 * @throws {InvalidRLP}
 */
const _validateNumericKindNumber = (num, context) => {
    if (!Number.isSafeInteger(num) || num < 0) {
        throw new sdk_errors_1.InvalidRLP('_validateNumericKindNumber()', `Validation error: Number in ${context} must be a safe and non-negative integer.`, {
            context,
            data: {
                num
            }
 ...
 * @private
 */
const _validateNumericKindString = (str, context) => {
    const isHexUInt = HexUInt_1.HexUInt.isValid0x(str);
    const isDecimal = FixedPointNumber_1.FixedPointNumber.isNaturalExpression(str);
    // Ensure the string is either a hex or decimal number.
    if (!isHexUInt && !isDecimal) {
        throw new sdk_errors_1.InvalidRLP('_validateNumericKindString()', `Validation error: String in ${context} must represent a non-negative integer in hex or decimal format.`, {
            context,
            data: {
                str
            }
    ...
    if (isHexUInt && str.length <= 2) {
        throw new sdk_errors_1.InvalidRLP('_validateNumericKindString()', `Validation error: Hex string number in ${context} must be of valid length.`, {
            context,
            data: {
                str
            }
 ...
 * @private
 */
const assertValidNumericKindBuffer = (buf, context, maxBytes) => {
    // If maxBytes is defined, ensure buffer length is within bounds.
    if (maxBytes !== undefined && buf.length > maxBytes) {
        throw new sdk_errors_1.InvalidRLP('assertValidNumericKindBuffer()', `Validation error: Buffer in ${context} must be less than ${maxBytes} bytes.`, {
            context,
            data: {
                buf,
                maxBytes
    ...
    if (buf[0] === 0) {
        throw new sdk_errors_1.InvalidRLP('assertValidNumericKindBuffer()', `Validation error: Buffer in ${context} must represent a canonical integer (no leading zeros).`, {
            context,
            data: {
                buf,
                maxBytes
...
exports.assertValidNumericKindBuffer = assertValidNumericKindBuffer;
/**
 ...
 * @throws {InvalidRLP}
 */
const encodeBigIntToBuffer = (bi, maxBytes, context) => {
    if (bi === 0n)
        return Uint8Array.from([]);
    const hex = Hex_1.Hex.of(bi).digits;
    if (maxBytes !== undefined && hex.length > maxBytes * 2) {
        throw new sdk_errors_1.InvalidRLP('encodeBigIntToBuffer()', `Validation error: Encoded number in ${context} must fit within ${maxBytes} bytes.`, {
            context,
            data: {
                hex,
                maxBytes
    ...
    return Hex_1.Hex.of(hex).bytes;
};
exports.encodeBigIntToBuffer = encodeBigIntToBuffer;
/**
 ...
 */
const decodeBufferToNumberOrHex = (buffer) => {
    if (buffer.length === 0)
        return 0;
    const bi = Hex_1.Hex.of(buffer).bi;
    const num = Number(bi);
    // Return number or hex based on integer safety
    return Number.isSafeInteger(num) ? num : '0x' + bi.toString(16);
};
exports.decodeBufferToNumberOrHex = decodeBufferToNumberOrHex;
