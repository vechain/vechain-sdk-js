...
describe('mnemonic', () => {
    ...
    test('validate', () => {
        ...
        expect(mnemonic.validate(mnemonic.generate().phrase.split(' '))).toEqual(true)
    })
    test('derive', () => {
        expect(mnemonic.derivePrivateKey(words).toString('hex')).toEqual('27196338e7d0b5e7bf1be1c0327c53a244a18ef0b102976980e341500f492425')
    })
    test('hdNode', () => {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            expect(address.fromPublicKey(child.publicKey).slice(2)).toEqual(addresses[i])
            expect(child.address).toEqual('0x' + addresses[i])
            expect(secp256k1.derive(child.privateKey!).toString('hex')).toEqual(child.publicKey.toString('hex'))
        }
