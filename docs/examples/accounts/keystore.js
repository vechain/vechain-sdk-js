...
const newKeyStore = keystore.encrypt(privateKey, new TextEncoder().encode(keyStorePassword.normalize('NFKC')));
// @NOTE the `encrypt` function wipes private key and password after use.
// 3 - Throw for wrong password
const recoveredPrivateKey = keystore.decrypt(newKeyStore, new TextEncoder().encode(keyStorePassword.normalize('NFKC')));
