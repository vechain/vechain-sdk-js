...
const sender = {
    ...
    privateKey: HexUInt.of('ea5383ac1f9e625220039a4afac6a7f868bf1ad4f48ce3a1dd78bd214ee4ace5').bytes
};
const receiver = {
    address: '0x9e7911de289c3c856ce7f421034f66b6cde49c39',
    privateKey: HexUInt.of('1758771c54938e977518e4ff1c297aca882f6598891df503030734532efa790e').bytes
};
...
// 2.3 - Calculate gas
const gasResult = await thorSoloClient.transactions.estimateGas(clauses, sender.address);
...
const senderBalanceBefore = (await thorSoloClient.accounts.getAccount(Address.of(sender.address))).balance;
const receiverBalanceBefore = (await thorSoloClient.accounts.getAccount(Address.of(receiver.address))).balance;
console.log('Sender balance before:', senderBalanceBefore);
console.log('Receiver balance before:', receiverBalanceBefore);
// 4 - Send transaction
const sentTransaction = await thorSoloClient.transactions.sendRawTransaction(raw);
...
const newBalanceSender = await Poll.SyncPoll(async () => (await thorSoloClient.accounts.getAccount(Address.of(sender.address)))
    .balance).waitUntil((newBalance) => {
    return newBalance !== senderBalanceBefore;
});
// New balance of receiver (wait until the balance is updated)
const newBalanceReceiver = await Poll.SyncPoll(async () => (await thorSoloClient.accounts.getAccount(Address.of(receiver.address)))
    .balance).waitUntil((newBalance) => {
