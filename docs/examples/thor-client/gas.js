...
// Estimate gas
const gasResult = await thorSoloClient.transactions.estimateGas(clauses, senderAccount.address, options);
...
// 5 - Sign transaction
const rawNormalSigned = Transaction.of(transactionBody).sign(senderAccount.privateKey).encoded;
// 6 - Send transaction
const send = await thorSoloClient.transactions.sendRawTransaction(HexUInt.of(rawNormalSigned).toString());
